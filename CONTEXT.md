...
**Handling Option**:
A proposed way to address one Judge Issue on the Aggregator Report (not a disagreement cluster as a whole). Offered for CRITICAL and MAJOR; MINOR Issues are listed without Handling Options. If there are no CRITICAL or MAJOR Issues, coverage holds with zero Handling Options and phrasing is skipped. Otherwise each such Issue has at least one non-Other Handling Option, planted from a closed Finding-Kind catalog when the report is composed (template label and prose). Catalog targets are only gap, contribution, claims, and experiment_plan; idea_decomposition is Other-only. Phrasing generate may rewrite label and prose; it must not drop required rows, add non-catalog targets, or invent Other. A failed phrasing generate leaves the templates; that is still a successful Aggregator generate, not Confirm. Choosing one is a Decision (PICK). If the target Node Head is current, PICK reopens it (same idea as EDIT) and does not mark it Stale; the Account leaves Independent judges. It writes a prose suggested patch (and target Card ids) onto that node's Working Draft narrative; it does not patch Card bodies, the Spec Version, Severity, or Judge Runs. Several Handling Options may share one Judge Issue and target different Workflow Nodes. Other is a Handling Option whose prose and target Workflow Node are supplied by the Account (gap, contribution, claims, experiment_plan, or idea_decomposition); the Aggregator LLM does not invent Other. Generate, Confirm, Stale, and related Judge re-runs follow the existing loop; Judges do not auto-run. PICK and Other are offered on the working Aggregator Report once coverage holds, including while phrasing is in-flight; Confirm Aggregator is blocked until phrasing completes or fails. Handling Options may be skipped for Spec Artifact export; skipping CRITICAL does not make Readiness pass.
_Avoid_: Grilling Option, override, export anyway, patch (when you mean the Decision), Apply suggestion, stub (as a domain name)

**Aggregator Report**:
The Aggregator's composed output from the five current Judge Runs: grouped Judge Issues, consensus vs disagreement, Readiness flags, and Handling Options. It copies Severity, originating Judge, and Judge Issue Grounds from Judge Runs and must not rewrite them. Every Aggregator generate is compose, plant catalog Handling Options, then phrase when any CRITICAL or MAJOR Issue exists; there is no phrase-only generate. Compose persists Issues, Readiness flags, and catalog options on the working report. Phrasing is the second half of that same Aggregator generate, auto-started after compose. It is not a sixth Judge and not Confirm. A later successful Judge generate that leaves five heads current, or an Account-initiated Aggregator generate, tears down the unconfirmed working report (Issues and Handling Options), aborts in-flight phrasing, and runs compose plus phrasing again. LLM copy may phrase Handling Options; it must not change Severity, invent a majority verdict, or be the source of Grounds. Independent judges shows Judge Issues (with originating Judge and Grounds) and Handling Options here, not as five full Judge lists. Generate starts when all five Judge heads are current; Confirm Aggregator is still an Account Decision, is blocked while phrasing is in-flight, and advances to Readiness.
