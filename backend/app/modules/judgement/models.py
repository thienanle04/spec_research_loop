...
_CLUSTER_VALUES = "'consensus', 'disagreement'"


class AggregatorIssue(Base):
    """A Judge Issue copied onto a working or frozen Aggregator Report."""

    __tablename__ = "judgement_aggregator_issues"
    __table_args__ = (
        CheckConstraint(
            f"finding_kind IN ({_FINDING_KIND_VALUES})",
...
class HandlingOption(Base):
    """Handling Option copy on a working or frozen Aggregator Report."""

    __tablename__ = "judgement_handling_options"
    __table_args__ = (
        UniqueConstraint(
            "session_id",
            "stage_revision_id",
    ...
    aggregator_issue_id: Mapped[UUID] = mapped_column(
...
class AggregatorScore(Base):
    """Conference criterion scores copied onto an Aggregator Report."""

    __tablename__ = "judgement_aggregator_scores"
    __table_args__ = (
        UniqueConstraint(
            "session_id",
            "stage_revision_id",
